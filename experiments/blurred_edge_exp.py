...
FOLDER = os.path.abspath("./blurred_edge_exp")
...
for lbl, stim in zip(['edge_on', 'edge_off'], [edge_on, edge_off]):
    for velo in [-SPEED, SPEED]:
        oris = np.arange(num_frames) * velo / hc.scheduler.freq
        for std in stds:
            starts = [
                [stim.blur_image, std],
                [stim.switch, True],
                ...
                [print, f"stim: {lbl},\tvelo: {velo},\tstd: {std}"]
            ]
            middles = [
            ...
            ends = [
                ...
                [tracker.add_test_data, hc.window.record_stop,
                    {'stop_test': time.time, 'stim': lbl,
                    'velo': velo, 'std': std}, True],
                [hc.window.reset_rot],
            ]
