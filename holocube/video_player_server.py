...
class VideoGUI(QtWidgets.QMainWindow):
    def __init__(self, img_height=600, img_width=500, display_rate=15,
    ...
    def update_north(self, north=None):
        ...
        if np.isnan(north):
            ...
            if not self.tracking_active:
                self.tracking_active = True
                self.head_line.setPen(self.head_pen)
            # update position of head pin based on the north and inner radius
...
class FrameUpdater():
    def __init__(self, height, width, display_rate=15., buffer_fn='_buffer.npy'):
